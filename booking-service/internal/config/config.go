...
func Load() *Config {
	...
	if dbURL == "" {
		dbURL = fmt.Sprintf(
			"host=%s user=%s password=%s dbname=%s port=%s sslmode=require search_path=booking_service",
	...
	return &Config{
		DBURL:      dbURL,
		DBHost:     firstNonEmpty(os.Getenv("DB_HOST"), viper.GetString("db.host")),
		DBUser:     firstNonEmpty(os.Getenv("DB_USER"), viper.GetString("db.user")),
		DBPassword: firstNonEmpty(os.Getenv("DB_PASSWORD"), viper.GetString("db.password")),
